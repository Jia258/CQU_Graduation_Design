function [Optimal_X, Optimal_F,t_all] = Split_COASAHPSO_Fast()
    N = evalin('base', 'N');
    %% 种群参数
    ...
    Tfit = zeros(1, popsize);
    CumFit = zeros(1, popsize);
    % 假定一个全局最优
    gbest = zeros(N, 1);
    obj_gbest = Obj(gbest);
    ...
    for iter = 1:iter_max
        ...
        c = u1 > u2;
        V_new = c .* ones(N, popsize) .* (pbest - X) + (~c .* ones(N, popsize)) .* (gbest_plus - X);
